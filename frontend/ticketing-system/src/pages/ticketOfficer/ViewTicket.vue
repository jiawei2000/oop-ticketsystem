...
const fetchTicketInfo = async (eventId, transactionId) => {
    try {
        const response = await axios.get(`ticketofficer/tickets/${transactionId}`);
...
const goBack = () => {
    router.push({ name: "ticketOfficer-issue-ticket" });
};
...
onMounted(() => {
    ...
    if (eventId && transactionId) {
        fetchTicketInfo(eventId, transactionId);
    }
});
                    ...
                    <VCard :class="{ 'mb-3': index % 2 !== 0 }">
                        <div
                            ...
                            <div class="ma-auto pa-5">
                                <img v-if="ticket.qrCode" :src="ticket.qrCode" alt="QR Code"
                            ...
                            <div :style="{ 'width': '420px' }">
                                ...
                                <VCardText>
                                    <span class="font-weight-bold">Timing :</span> {{ ticket.eventDate }} {{
                    ticket.eventTime }}
                                </VCardText>
                                <VCardText>
