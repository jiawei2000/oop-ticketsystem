                        ...
                        <tr v-for="transaction in transactions" :key="transaction.transaction.transactionId">
                            ...
                            <td>
                                <v-btn class="mr-2" variant="outlined"
                                    @click="redirectToViewTicketPage(transaction.transaction.transactionId, transaction.transaction.eventId)">View
                                    Details</v-btn>
                                <v-btn
                                    v-if="transaction.transaction.status !== 'Refunded' && transaction.transaction.status !== 'Cancelled'"
                                    variant="outlined" color="red"
                                    @click="confirmRefund(transaction.transaction.transactionId)">Refund
                                    Transaction</v-btn>
                            </td>
                        </tr>
                    </tbody>
...
const fetchTransactions = async () => {
    try {
        ...
        //Remove wrong physical transactions
        for (let i = 0; i < transactions.value.length; i++) {
            if (transactions.value[i].tickets[0].type === "Physical") {
